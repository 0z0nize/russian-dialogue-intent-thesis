...
Текущая intent-модель — single-task RuBERT, обученная на **одиночных
репликах**, поэтому для длинных диалогов один общий прогон даёт лишь один
доминирующий intent (часто это intent самой длинной/последней реплики).
На вкладке «Текст» рядом с кнопкой `Анализировать как один текст`
доступна кнопка **`Анализировать по репликам`** — она парсит вход на
отдельные реплики и предсказывает intent для каждой.

Поддерживаемые форматы тегов спикеров:

- `#Person1#: ...`, `#Person2#: ...` (DialogSum / DialogSum-RU);
- `Person1: ...`, `Speaker 2: ...`;
- `Говорящий 1: ...`, `Спикер2: ...`, `Собеседник 1: ...`.

Если теги не найдены, парсер режет вход по непустым строкам, а поле
«Говорящий» остаётся пустым. Результат отображается в таблице
`№ / Говорящий / Реплика / Интент / Уверенность / Top-3`. Модели НЕ
перезагружаются на каждую реплику — переиспользуются уже загруженные
артефакты, поэтому стоимость одной реплики — это один forward-pass RuBERT.

Кнопка `Анализировать как один текст` сохраняется для коротких реплик и
демонстрации единого пайплайна intent + topic + summary.
